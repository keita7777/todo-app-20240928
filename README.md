...
- 認証状態によって表示する要素を切り替え
- タイトル検索
  テキストを入力して検索をクリックすると/todos に遷移し、入力したテキストが含まれる TODO が表示される
  検索状態にあるとき（URL に query=XXX があるとき）のみ「検索条件をリセット」ボタンが表示される
- プロフィール画像クリックでメニュー開閉
...
- 削除をクリックするとモーダルウィンドウが開く
  モーダルウィンドウの削除をクリックするとデータベースから TODO が削除される
  キャンセルをクリックするとモーダルウィンドウが閉じる
